...
let page = {
  ...
  callHer: function (e) {
    let curObj = e.currentTarget.dataset;
    curObj.tel && wx.makePhoneCall({
      phoneNumber: curObj.tel,
      complete: res => {
        console.info(res);
      }
    });
  ...
  onLoad: function (options) {
      ...
      wx.request({
        ...
        complete: res => {
          ...
          if (typeof resData === 'object') {
            ...
            resData.mapx = latLng.lng;
            resData.mapy = latLng.lat;
            // 初始化页面数据
            that.setData({
              dataLoad: true,
              pageInfo: resData
    ...
    let zbTask = new Promise((resolve, reject) => {
      wx.request({
        ...
        data: {
          city: encodeURIComponent(encodeURIComponent(that.params.cityname)),
          newcode: that.params.houseid
        },
        complete: res => {
          let resData = res.data;
      ...
      let xqTask = new Promise((resolve, reject) => {
        wx.request({
          ...
          data: {
            city: encodeURIComponent(that.params.cityname),
            id: that.params.houseid,
            x: app.userPosition.location.bdlng || (+this.data.pageInfo.mapx),
            y: app.userPosition.location.bdlat || (+this.data.pageInfo.mapy)
          },
          complete: res => {
            let resData = res.data;
            if (typeof resData === 'object') {
