...
function getReferrer() {
    let pages = getCurrentPages(), result = '';
    // 若有referrer
    if (pages.length > 1) {
...
function getStatisticsParam(datatype) {
    let referrer = encodeURIComponent(getReferrer()), globalid = getGlobalID(), pageid = encodeURIComponent(getCurrentPage().route), userid = '';
    let app = getApp();
...
/**
*  对象的深复制
* @param  Number Null Undefined String Boolean; Object, Array, Date, RegExp, Function
* @returns Number  Null Undefined String Boolean; Object, Array, Date, RegExp, Function
 */
function deepCopy(source) {
    let result, oClass = getDataType(source);
    // 确定result的类型
    if (oClass === "Object") {
        result = {};
...
function request(option = {}) {
    ...
    if (option && option.data) {
        ...
        let dataType = typeof data, statisData = null;
        // 根据data类型不同处理
