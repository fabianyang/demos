...
let page = {
    ...
    shareInfoFn: function () {
        return { desc: '轻松评估房价走势', title: '房天下+' };
    },
    data: {
        // 判断定位数据是否完毕
    ...
    onLoad: function (params) {
        ...
        // 判断是否是城市切换页
        if (this.params.isCitySelect) {
            this.citySelectFlag = true;
            delete this.params.isCitySelect
        }
    ...
    init: function (isReady) {
        ...
        Promise.all([pageInfoTask, searchInfoTask]).then(res => {
            ...
            isReady && !this.filterFlag && !this.citySelectFlag && this.showPositionTip();
        }).then(() => {
            // 小区列表数据
    ...
    checkPosition: function () {
        ...
        let time = setInterval(() => {
            if (app.positionReady) {
                this.paramsQuery = Object.assign({}, this.params);
                if (!this.filterFlag && !this.citySelectFlag && !this.params.keyword  && app.userPosition.location) {
                    let location = app.userPosition.location;
                    this.paramsQuery.lng = location.lng;
                    this.paramsQuery.lat = location.lat;
                    this.paramsQuery.distance = 2;
                ...
                !this.filterFlag && !this.citySelectFlag && this.showPositionTip();
                clearInterval(time);
            }
        }, 1000);
    },
...
// 合并模块方法
utils.assign(page, filter, search, citySelect, common, share);
// 渲染页面
Page(page);
