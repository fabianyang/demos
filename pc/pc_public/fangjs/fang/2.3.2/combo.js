/**
 * combo.js for fang.js
 * The fang.js plugin for concatenating HTTP requests
 */

(function(global) {
    'use strict';
    var fangjs = global.fangjs;
    var Module = fangjs.Module
    var FETCHING = Module.STATUS.FETCHING

    var data = fangjs.settings;
    var comboHash = data.comboHash = {}

    var comboSyntax = ["??", ","];
    var comboMaxLength = 2000;
    var comboExcludes;
    var comboSuffix = function() {
        return '?_' + data.img_ver;;
    };

    fangjs.on("load", setComboHash);
    fangjs.on("fetch", setRequestUri);
    ...
    // For test
    if (data.test) {
        var test = fangjs.test || (fangjs.test = {})
