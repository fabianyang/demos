    ...
    SmsLogin.prototype = {
        ...
        showImgCode: function (succFn, errFn) {
            ...
            if (imgCodeBox.length < 1) {
                ...
                elementSubmit.on('click', function () {
                    ...
                    elelmentFlog.hide();
                    that.check(that.options.mobilephone, that.options.mobilecode, succFn, errFn);
                });
                // 点击更换验证码
        ...
        getImgCodeSrc: function () {
            ...
            var time = Date.now(),
                imgSrc = '';
            if (time % 2) {
                imgSrc = 'http://captcha.fang.com/Display?type=soufangbang&width=100&height=32&r=' + time;
            } else {
                imgSrc = 'http://captcha.fang.com/Display?r=' + time;
            }
            return imgSrc;
        }
    }
    window.smsLogin = new SmsLogin();
    return smsLogin;
})
;
