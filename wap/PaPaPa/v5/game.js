...
Game.prototype.bindEvent = function () {
    ...
    function deviceMotionHandler(e) {
        ...
        if ((t1 - t0) > 1000) {
            ...
            if (Math.abs(ay) > 10) {
                shakeTimes++;
                shakePower = (shakePower + ay) / shakeTimes;
            }
            document.title = shakePower + ',' + shakeTimes;
            t0 = t1;
        }
    }
...
Game.prototype.initStage = function () {
    ...
    rail.draw(context);
    // context.drawImage(rail.spirit, 0, 0, canvas.width, canvas.height - 329 * scale.height, 0, 329 * scale.height, canvas.width, canvas.height - 329 * scale.height);
...
Game.prototype.update = function () {
    ...
    if (ball.state === 'up' && ball.y < 329 * scale.height){//329 * scale.height) {
        if (!rail.hasMoved) {
            rail.isMoving = true;
            rail.move();
        }
...
Game.prototype.levelUp = function (count) {
