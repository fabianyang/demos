$(function () {
    ...
    var resultReply = {
        right: {
            count: 0,
            text: ['友谊的小船要扬帆起航了', '厉害了，我的哥！', '亦可赛艇！', '朋友，你知道的太多了', '老铁666！看来必须得请你吃顿饭了。']
        },
        wrong: {
            count: 0,
            text: ['我们的基情破裂了', '我可能交了个假朋友。', '你的好友已下线。', '友尽，出门左转不送', '对方并不想理你，并向你丢了一坨']
        }
    }
    ...
    function sendR(tof) {
        ...
        if (tof === 'wrong' && resultReply[tof].count === 4) {
    ...
    button.eq(3).on('click', function () {
        var cookie = getCookie('itemUUID');
        if (cookie && cookie === itemUUID) {
            window.location.href = url + '&m=redirtScores&subjectOpenId=' + subjectOpenId + '&openId=' + openId + '&itemUUID=' + itemUUID;
        } else {
