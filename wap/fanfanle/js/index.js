...
$(function () {
    ...
    var winBox = $('.js_win'),
        loseBox = $('.js_lose'),
        noChargeCount = $('.js_noChargeCount'),
        noPlayCount = $('.js_noPlayCount'),
        sttext = $('.sttext'),
        title01 = $('.title01');
    ...
    var route = {
        ...
        recharge: 'https://mstore.fang.com/flowrecharge/index.shtml'
        // 新需求，中积分不再跳转积分商城。
    ...
    function requestResult() {
        ...
        $.get(url, function (data) {
            ...
            noWinReason.val(dataRoot.noWinReason);
            // 有可能请求积分接口失败
            if (!dataRoot.canWin) {
            ...
            if (dataRoot.isGetPrize) {
                updatePageStatus(dataRoot.freePlay, dataRoot.surplusPoints, dataRoot.shoppointnumber, dataRoot.flag);
     ...
     */
    function updatePageStatus(freeCount, remainingPoints, bonusPoints, flag) {
        canWin.val(+remainingPoints);
        surplusPlayCount.val(+freeCount);
     ...
     * 去充值蒙层，隐藏蒙层。
     */
    noChargeCount.on('click', function (e) {
        var $target = $('a.no1:not(.close)');
        if ($target.is(e.target) || $target.has(e.target).length !== 0) {
            window.location.href = route.recharge;
        } else {
            $(this).hide();
        }
        return false;
