...
import settings from '../settings.js'
  export default {
    ...
    methods: {
      addFriend: function() {
        axios.post(settings.server + '/getUserInfo', { username: this.add_friend }).then((res) => {
          ...
          if(result.username !== this.loginId) {
            ...
            if(!norepeatFriend) {
              this.show = false
              axios.post(settings.server + '/getUserInfo', { username: this.loginId }).then((res) => {
                ...
                axios.post(settings.server + '/updateUser', new_user).then((res) => {
                  let result = res.data
                   if(result.status === 'OK') {
                      alert('添加成功！')
                    } else {
                ...
                axios.post(settings.server + '/updateUser', new_login).then((res) => {
                  let result = res.data
                })
              })
            }
