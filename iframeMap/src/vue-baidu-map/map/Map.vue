...
import {checkType} from '@vue-baidu-map/base/util.js'
...
export default {
  ...
  methods: {
    ...
    getMapScript () {
      if (!global.BMap) {
        const libSrc = this.libSrc || this._BMap().libSrc;
        global.BMap = {}
        global.BMap._preloader = new Promise((resolve, reject) => {
          ...
          global.BMap.apiLoad = function() {
              resolve(global.BMap)
              global.document.body.removeChild($script)
              global.BMap._preloader = null
              global.BMap.apiLoad = null
          }
          const $script = document.createElement('script')
          global.document.body.appendChild($script)
          // $script.src = `//api.map.baidu.com/api?v=2.0&ak=${ak}&callback=_initBaiduMap`
          $script.src = libSrc;
          // require('@vue-baidu-map/BMap/20170809.js')
        })
